...
### Post-first-CI-run corrections
Two design gaps surfaced only once real CI jobs ran (not reproducible locally, since local dev has neither of these environment differences):

1. **Missing `lock-check` dependency.** Each `lint`/`typecheck`/`test`/`build`/`audit`/`check-docs` target originally depended only on `up-d`, not on `lock-check`. Locally that's invisible — the same container/volume persists across separate `make` invocations, so `node_modules` was always already populated from a prior command. In CI, each job is a fresh runner with its own fresh named volume — nothing had installed dependencies first, so every migrated job except `lock-check` itself failed with `sh: vitest: not found` (or the lint/tsc equivalent). Fix: those targets now depend on `lock-check` (not `up-d` directly), so each is self-sufficient regardless of invocation order or fresh state.
2. **Bind-mount UID mismatch on the CI runner.** `docker-compose.yml`'s `app` service ran as `user: node` (non-root). On a typical single-user Linux dev box that UID happens to match the host directory owner, so writes into the bind-mounted `.:/app` silently succeed — coincidence, not a guarantee. On a GitHub Actions runner, `actions/checkout` writes the workspace as the runner's own user, a different UID that the container's `node` user has no write access to — `pnpm install` failed with `EACCES` writing a temp file directly under `/app` (unrelated to the already-fixed `node_modules` volume permissions). Fix: `app` now runs as `user: root` in `docker-compose.yml`, the documented exception in `development-standards.md` §1 for a dev container never reachable outside localhost — sidesteps UID-matching entirely rather than trying to reconcile it per-host. The `Dockerfile.dev` chown step added for the `node_modules` mount point (for the non-root case) was removed as dead weight once root doesn't need it.
