...
- `Makefile`: nuevos targets `up`/`up-d` (`docker compose up [-d] --build --wait`) y targets Docker-first por categoría (`lint`, `typecheck`, `test`, `build`, `audit`; `lock-check`/`check-docs` reescritos) que delegan a `docker compose exec app <cmd>` en vez de invocar `pnpm`/scripts directo en el host — `validate` pasa a componerlos. `check-docker-cmd-shell-form`/`docker-port-smoke-test` quedan sin tocar (host-invocados a propósito, no dependen de Node/pnpm). `docker-compose.yml` agrega un volumen nombrado `node_modules` (junto al ya existente `pnpm-store`) para que `node_modules` no se bind-monte al host — evita mismatches de binarios nativos entre el host y la imagen Linux. `Dockerfile.dev` pre-crea y chownea `/app/node_modules` para que el usuario non-root `node` pueda escribir en ese volumen. `.github/workflows/ci.yml`: los jobs `lock-check`, `lint`, `typecheck`, `test`, `build` y `audit-and-docs` dejan de usar `actions/setup-node`/`pnpm/action-setup` y corren `make <target>` en su lugar (`timeout-minutes` sube de 10 a 15 en esos 6 jobs); `docker-checks`/`trivy-fs`/`docker-image` quedan sin cambios. `.husky/pre-push` y `.husky/pre-merge-commit`: el fast-path de rama de feature pasa de `pnpm run typecheck` a `make typecheck`. Ningún contribuidor necesita Node/pnpm instalado en el host para correr `make validate`, solo Docker (issue #53).
